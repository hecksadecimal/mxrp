...
def forgot_password_post():
    ...
    if g.redis.get("reset_password_limit:%s" % request.headers.get("CF-Connecting-IP", request.remote_addr)):
    ...
    g.redis.setex("reset_password_limit:%s" % request.headers.get("CF-Connecting-IP", request.remote_addr), 86400, 1)
...
def register_post():
    ...
    if g.redis.exists("register:" + request.headers.get("CF-Connecting-IP", request.remote_addr)):
    ...
    new_user = User(
        ...
        group="new",
        last_ip=request.headers.get("CF-Connecting-IP", request.remote_addr),
    )
    new_user.set_password(request.form["password"])
    g.db.add(new_user)
    g.db.flush()
    g.redis.set("session:" + g.session_id, new_user.id, 2592000)
    g.redis.setex("register:" + request.headers.get("CF-Connecting-IP", request.remote_addr), 86400, 1)
