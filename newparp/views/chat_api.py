...
def user_action():
    ...
    if action == "kick":
        ...
        send_message(g.db, g.redis, Message(
            ...
            name=g.chat_user.name,
            text=ban_message,
        ), g.user_list, raw_text=ban_message)
...
def set_flag():
    ...
    ), g.user_list, raw_text=message % (g.chat_user.name, g.chat_user.acronym))
...
def set_topic():
    ...
    if topic == "":
        ...
        ), g.user_list, raw_text="%s [%s] removed the conversation topic." % (
                g.chat_user.name, g.chat_user.acronym,
            ))
    else:
        send_message(g.db, g.redis, Message(
        ...
        ), g.user_list, raw_text="%s [%s] changed the topic to \"%s\"" % (
                g.chat_user.name, g.chat_user.acronym, topic,
            ))
...
def set_info():
    ...
    ), g.user_list, raw_text="%s [%s] edited the chat information." % (
            g.chat_user.name, g.chat_user.acronym,
        ))
...
def save():
    ...
    if g.chat_user.name != old_name or g.chat_user.acronym != old_acronym:
        if g.chat_user.computed_group == "silent":
            ...
            ), g.user_list, raw_text=("%s [%s] is now %s [%s].") % (
                    old_name, old_acronym,
                    g.chat_user.name, g.chat_user.acronym,
                ))
...
def look_up_user():
    ...
    send_temporary_message(
        ...
        "User number %s is [url=%s]#%s %s[/url], last IP [url=%s]%s[/url]." % (
            chat_user.number,
