		...
		"chat": function(chat, user, character_shortcuts, latest_message_id, latest_time, token) {
			...
			var text_commands = [
				{
					...
					"action": function(match) {
						$.post("/chat_api/send", { "chat_id": chat.id, "type": "me", "text": match[1], "raw_text": match[1] });
					},
				},
				{
					"regex": /^block (\d+)($|\s.*$)?/,
				...
				},
				{
					...
					"action": function(match) {
						...
						if (match[1] == "nsfwe") { match[1] = "nsfw-extreme"; }
			...
			var text_input = $("input[name=text]").keydown(function() {
				...
				if (user.meta.show_preview) {
					...
					} else if (text.substr(0, 7) == "http://" || text.substr(0, 8) == "https://" || ["((", "[[", "{{"].indexOf(text.substr(0, 2)) != -1 || ["))", "]]", "}}"].indexOf(text.substr(-2, 2)) != -1) {
			...
			var send_form = $("#send_form").submit(function() {
				...
				} else if (["((", "[[", "{{", "//"].indexOf(data.text.substr(0, 2)) != -1 || ["))", "]]", "}}", "\\\\"].indexOf(data.text.substr(-2, 2)) != -1) {
			...
			var abscond_button = $("#abscond_button").click(function() {
				if (status == "chatting") {
					if (confirm("Are you sure you want to leave the chat?")) { disconnect(); }
